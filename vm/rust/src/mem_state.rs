...
use blockifier::{
    ...
    state::state_api::{StateReader, State, StateResult},
};
...
impl StateReader for MemState {
    fn get_storage_at(&mut self, contract_address: ContractAddress, key: StorageKey) -> StateResult<StarkFelt> {
        if let Some(value) = STORAGE_CACHE.lock().unwrap().cache_get(&(contract_address, key)) {
    ...
    fn get_class_hash_at(&mut self, contract_address: ContractAddress) -> StateResult<ClassHash> {
        if let Some(class_hash) = CLASS_HASH_CACHE.lock().unwrap().cache_get(&contract_address) {
    ...
    fn get_compiled_class_hash(&mut self, class_hash: ClassHash) -> StateResult<CompiledClassHash> {
        if let Some(compiledClassHash) = COMPILED_CLASS_HASH.lock().unwrap().cache_get(&class_hash) {
            return Ok(compiledClassHash.clone())
        }
        return Err(StateError::UndeclaredClassHash(*class_hash))
    }
}
...
impl State for MemState {
    fn set_storage_at(&mut self, contract_address: ContractAddress, key: StorageKey, value: StarkFelt) {
        let _ = STORAGE_CACHE.lock().unwrap().cache_set((contract_address, key), value);
    }
    ...
    fn increment_nonce(&mut self, contract_address: ContractAddress) -> StateResult<()> {
        ...
        let _ = NONCE_CACHE.lock().unwrap().cache_set(contract_address, next_nonce);
        Ok(())
    }

    fn set_class_hash_at(&mut self, contract_address: ContractAddress, class_hash: ClassHash) -> StateResult<()> {
        let _ = CLASS_HASH_CACHE.lock().unwrap().cache_set(contract_address, class_hash);
        Ok(())
    }

    fn set_contract_class(&mut self, class_hash: &ClassHash, contract_class: ContractClass) -> StateResult<()> {
        let _ = CLASS_CACHE.lock().unwrap().cache_set(*class_hash, CachedContractClass{definition: contract_class, cached_on_height: 0 });
        Ok(())
    }

    fn set_compiled_class_hash(&mut self, class_hash: ClassHash, compiled_class_hash: CompiledClassHash) -> StateResult<()> {
        let _ = COMPILED_CLASS_HASH.lock().unwrap().cache_set(class_hash, compiled_class_hash);
        Ok(())
    }
